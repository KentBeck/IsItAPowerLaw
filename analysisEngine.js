...
class AnalysisEngine {
  ...
  analyzeSingle(analyzerName, data) {
    ...
    if (!validation.valid) {
      throw new Error(
        `Data validation failed for ${analyzerName}: ${validation.message}`
      );
    }
  ...
  analyzeMultiple(data, analyzerNames = null) {
    ...
    if (analyzersToUse.length === 0) {
      throw new Error(
        "No analyzers specified and no default analyzers registered"
      );
    }
    ...
    const bestFit = DistributionUtils.getBestFit(rankedResults);
  ...
  generateSummary(rankedResults, bestFit, errors) {
    ...
    if (bestFit) {
      ...
      recommendation = `The analysis suggests your data follows a ${
        bestFit.displayName
      } distribution with ${(confidence * 100).toFixed(1)}% confidence.`;

      // Add information about other distributions tested
      if (rankedResults.length > 1) {
        const alternatives = rankedResults
          .slice(1, 3)
          .map(
            (r) => `${r.displayName} (${(r.confidenceScore * 100).toFixed(1)}%)`
          )
          .join(", ");
      ...
      recommendation += `The closest match is ${topResult.displayName} with ${(
        confidence * 100
  ...
  preAnalyze(data) {
    ...
    if (suitable.length === 0) {
      recommendation =
        "No distribution analyzers can process this data. Please check data format and requirements.";
    } else if (suitable.length === 1) {
      recommendation = `Only ${suitable[0].displayName} analysis is possible with this data.`;
    } else {
      recommendation = `${
        suitable.length
      } distribution types can be analyzed: ${suitable
        .map((s) => s.displayName)
        .join(", ")}.`;
    }
