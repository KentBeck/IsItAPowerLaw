...
class PowerLawAnalyzer {
    ...
    analyze(dataWithCCDF) {
        ...
        if (logLogData.length < 3) {
            throw new Error('Not enough valid data points after log transformation. Need at least 3 points with non-zero CCDF.');
        }
        ...
        const goodnessOfFit = this.calculateGoodnessOfFit(logLogData, theoreticalValues, regression);
    ...
    calculateGoodnessOfFit(empiricalData, theoreticalData, regression) {
        ...
        const logLikelihood = this.calculateLogLikelihood(empiricalData, theoreticalData);
        ...
        return {
            rSquared,
            adjustedRSquared: 1 - (1 - rSquared) * (sampleSize - 1) / (sampleSize - numParameters - 1),
    ...
    validateData(data) {
        ...
        if (data.length < 5) {
            return { valid: false, message: 'Need at least 5 data points for power law analysis' };
        }
        ...
        if (validLogData.length < 3) {
            return { valid: false, message: 'Not enough data points with positive CCDF values for log transformation' };
        }
        ...
        if (valueRange < 10) {
            return { 
                valid: true, 
                message: 'Warning: Data spans less than one order of magnitude. Power law analysis may be unreliable.',
                warning: true
            };
        }
