...
class DistributionAnalyzer {
  constructor() {
    if (this.constructor === DistributionAnalyzer) {
      throw new Error(
        "DistributionAnalyzer is an abstract class and cannot be instantiated directly"
      );
    }
  ...
  getTheoreticalCCDF(xValues, parameters) {
    throw new Error(
      "getTheoreticalCCDF() method must be implemented by subclass"
    );
  }
  ...
  getDistributionSpecificPlot(data) {
    throw new Error(
      "getDistributionSpecificPlot() method must be implemented by subclass"
    );
  }
  ...
  calculateGoodnessOfFit(empiricalData, theoreticalData, additionalInfo = {}) {
    throw new Error(
      "calculateGoodnessOfFit() method must be implemented by subclass"
    );
  }
  ...
  isGoodFit(analysisResult) {
    ...
    if (gof.kolmogorovSmirnov && !gof.kolmogorovSmirnov.significant)
      return true;
  ...
  getConfidenceScore(analysisResult) {
    ...
    if (gof.rSquared !== undefined)
      return Math.max(0, Math.min(1, gof.rSquared));
    if (gof.confidenceScore !== undefined)
      return Math.max(0, Math.min(1, gof.confidenceScore));
  ...
  getSummary(analysisResult) {
    ...
    if (isGood) {
      return `Data shows ${confidenceLevel.toLowerCase()} evidence of ${
        this.displayName
      } behavior (confidence: ${(confidence * 100).toFixed(1)}%)`;
    } else {
      return `Data does not show strong evidence of ${
        this.displayName
      } behavior (confidence: ${(confidence * 100).toFixed(1)}%)`;
    }
  }
